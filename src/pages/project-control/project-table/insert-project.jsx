...
function InsertProject() {
	...
	const resetData = () => {
		const newData = table.map((item) => {
			if (item.key === 'project_code')
				return { ...item, value: tab.find((e) => e.isOpen).name };
			if (item.key === 'project_level') return { ...item, value: 'A' };
			return { ...item, value: '' };
		});
